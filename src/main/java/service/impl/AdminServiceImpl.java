...
public class AdminServiceImpl implements AdminService {
     ...
     */
    public List<Reservation> getReservation(String a_no)
    {
        //获取主键，通过主键查询
     ...
     */
    public List<Borrow> getBorrow(String a_no){
        borrowDao.setAllOverDueState();
        return borrowDao.getBorrowList(a_no);
    }
    ...
    {
        //归还设备
        int prime=returnDeviceDao.returnDevice(returnDevice);
        //若归还成功则改变借用状态
        if(prime!=0)
        {
            ...
            if (b_state == -1 || b_state == 0)
            {
                ...
                /*else
                {
                    if (prime == 0) errMsg.add("确认设备归还失败");
                }*/
        }
        return prime;
    }
    ...
    public int isAdmin(String name, String password) {
        ...
        int flag = adminDao.isAdmin(name,password);
        return flag;
    }
